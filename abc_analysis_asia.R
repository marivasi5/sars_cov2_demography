# ABC parameter inference for the asian population
#input: 
...
#INPUT
sim_file= '/home/mariav/synology/pavlos/cov/analysis/ABC/simulations/run_AS_large_ms.out'
obs_file='/home/mariav/synology/pavlos/cov/analysis/ABC/20200402/as_gisaid_cov2020_sequences_hc_20200402_oneline_fullseq_ns_20200402.fasta.stats'
run='as_large_FINAL'
tolerance=0.005
...
simulations  <- read.table(sim_file, header=T, fill = T)

#exoun oristei ta alpha kai theta anapoda apo ta alla ABC (kai sta plots apla vgainoun anapoda)
params=simulations[,c(2,1)]
...
#write.table(round(params_linear_bounds,2), file='final3/table_AS_lin_logit.txt', quote = F)

#===========  PARAMS CURVE ===============
axonasx=c(expression(log[10](alpha)), expression(log[10](theta))) #for ploting
...
ss <- sample(1:nrow(abc.res.lin$ss),  2500) #actually im gonna use all of the accepted sims as dots
...
#____parameter correlation ___

pdf('final3/as_cor.pdf')
par(mar=c(5.1,4.5,2,2))
plot(x=abc.res.lin$unadj.values[,1], y=abc.res.lin$unadj.values[,2], cex.lab=1.3, col= 'grey34',
     xlab=expression(log[10](alpha)), ylab=expression(log[10](theta)), frame.plot=FALSE )
dev.off()
